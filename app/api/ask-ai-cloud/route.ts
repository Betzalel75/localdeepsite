  ...
  try {
    ...
    (async () => {
      ...
      try {
        ...
        switch (provider) {
          case "deepseek":
            apiResponse = await callDeepSeekApi(model, messages, {
              stream,
              max_tokens,
              temperature,
            });
            break;
          case "google":
            apiResponse = await callGoogleApi(model, messages, {
              stream,
              max_tokens,
              temperature,
            });
            break;
          case "openai":
            apiResponse = await callOpenAIApi(model, messages, {
              stream,
              max_tokens,
              temperature,
            });
            break;
          case "anthropic":
            apiResponse = await callAnthropicApi(model, messages, {
              stream,
              max_tokens,
              temperature,
            });
            break;
          case "groq":
            apiResponse = await callGroqApi(model, messages, {
              stream,
              max_tokens,
              temperature,
            });
...
async function callGoogleApi(
  ...
  // CORRECTION : Nettoyer le nom du modèle
  const modelName = model.replace("google-", "");
  console.log("Using model:", modelName); // Pour debug
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    if (!apiResponse.ok) {
      ...
      throw new Error(
        `API Error: ${apiResponse.status} ${apiResponse.statusText} - ${errorData}`,
      );
    }
